...
export async function getTopScoringInterviews({ limit = 6 } = {}) {
  try {
    // Query feedback documents ordered by totalScore descending
    ...
    const results = [];
    // For each top feedback, fetch the interview document (and include userId)
    for (const doc of feedbackSnapshot.docs) {
      ...
      try {
        ...
        console.warn(`Failed to load interview ${interviewId}:`, ie);
      }
